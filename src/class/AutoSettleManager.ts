...
export default class AutoSettleManager extends AAskManager {
    ...
    this.list.forEach((ask) => {
      const price = priceTick.lastPrice;
			...
			if( cTime < priceTick.ticktime && ask.ProcStatus < 2){
				...
				if (ask.isUserSettle || (price - ask.GainPrice)*ask.ItemType > 0 
					...
					console.log('AutoSettle check:', price, ask.GainPrice, ask.LosePrice, ask.ItemType, (price - ask.GainPrice)*ask.ItemType, (price - ask.StopLose)*ask.ItemType);
					ask.Price = price;
